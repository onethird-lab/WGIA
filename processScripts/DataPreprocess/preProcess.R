...
# -----------------------------------------
# args=commandArgs(T)
args=c('a9f023p9gk7z5oy3fx50', 'D:/software/webdeveloper/PHPCUSTOM/wwwroot')
...
if (length(file_list) > 0) {
    ...
    file.rename(file_list[1], paste0(rootpath, args[1], "/matrix1",extension))  # Replace with the filename you want
}
# Unzip the zip files -- matirx2
unzip(paste0(rootpath, args[1], '/matrix2.zip'), exdir = paste0(rootpath, args[1], "/matrix2_zipped/"))
file_list <- list.files(path = paste0(rootpath, args[1], "/matrix2_zipped/"), recursive = TRUE, full.names = TRUE)
if (length(file_list) > 0) {
    # Select the first file found and move it to the current directory 
    extension <- get_extension(file_list[1])
    file.rename(file_list[1], paste0(rootpath, args[1], "/matrix2",extension))  # Replace with the filename you want
}
# Unzip the zip files -- matirx_pheno
unzip(paste0(rootpath, args[1], '/matrix_pheno.zip'), exdir = paste0(rootpath, args[1], "/matrix_pheno_zipped/"))
file_list <- list.files(path = paste0(rootpath, args[1], "/matrix_pheno_zipped/"), recursive = TRUE, full.names = TRUE)
if (length(file_list) > 0) {
    # Select the first file found and move it to the current directory 
    extension <- get_extension(file_list[1])
    file.rename(file_list[1], paste0(rootpath, args[1], "/matrix_pheno",extension))  # Replace with the filename you want
}
# Unzip the zip files -- matirx_cov
unzip(paste0(rootpath, args[1], '/matrix_cov.zip'), exdir = paste0(rootpath, args[1], "/matrix_cov_zipped/"))
file_list <- list.files(path = paste0(rootpath, args[1], "/matrix_cov_zipped/"), recursive = TRUE, full.names = TRUE)
if (length(file_list) > 0) {
    # Select the first file found and move it to the current directory 
    extension <- get_extension(file_list[1])
    file.rename(file_list[1], paste0(rootpath, args[1], "/matrix_cov",extension))  # Replace with the filename you want
}
...
process_file <- function(file_now, file_out, missing_value) {
  ...
  cat(paste0("   - the original file contains ", nrow(data_now), " markers and ", ncol(data_now), " samples.\n"))
  cat(paste0("   - There are ", sum(is.na(data_now)), " missing values be replaced with -999!\n"))
  data_now[is.na(data_now)] <- missing_value
  cat(paste0("   - the processed file contains ", nrow(data_now), " markers and ", ncol(data_now), " samples.\n"))
